...
const firebaseConfig = {
  ...
  async function loadEntries() {
    ...
    // Compute stats from all fetched entries
    entries.forEach((e, i) => {
  ...
  function generateCalendarGrid(days = 365) {
    if (calendarDays.length) return; // already generated
  ...
  function updateCalendarHeatMap(entries) {
    const dayTotals = {};
    entries.forEach(e => {
      const day = e.timestamp.toDate().toISOString().split("T")[0];
    ...
    calendarDays.forEach(div => {
      ...
      div.style.backgroundColor = seconds === 0
        ? "rgba(90,90,90,.1)"
        : getHeatMapColor(seconds / maxSeconds);
    });
  ...
  function getHeatMapColor(ratio) {
    ...
    if (ratio < 0.25) {
      const t = ratio / 0.25;
      return `rgba(${0},${Math.round(123 + t*132)},${Math.round(255 - t*255)},${alpha})`;
    } else if (ratio < 0.5) {
      const t = (ratio - 0.25)/0.25;
      return `rgba(${Math.round(t*255)},255,0,${alpha})`;
    } else if (ratio < 0.75) {
      const t = (ratio - 0.5)/0.25;
      return `rgba(255,${Math.round(255 - t*127)},0,${alpha})`;
    } else {
      const t = (ratio - 0.75)/0.25;
      return `rgba(255,${Math.round(128 - t*90)},${Math.round(0 + t*133)},${alpha})`;
    }
  }
